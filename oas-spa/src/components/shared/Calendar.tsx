...
/** 曜日ラベルをIntlで動的生成するためのベース日付（2024年1月7日=日曜日〜13日=土曜日） */
const WEEKDAY_BASE_DATES = [7, 8, 9, 10, 11, 12, 13].map(d => new Date(2024, 0, d));
...
export function Calendar({ selectedDate, onSelectDate, holidays, holidayNames, businessHours }: CalendarProps) {
  ...
  return (
    <div className="select-none">
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="text-center">
          <h3 className="text-lg font-heading font-semibold text-navy-700">
            {new Intl.DateTimeFormat(i18n.language, { month: 'long' }).format(new Date(viewYear, viewMonth))}
          </h3>
      ...
      {/* 曜日ヘッダー（ロケールに合わせてIntlで生成） */}
      <div className="grid grid-cols-7 mb-2">
        {WEEKDAY_BASE_DATES.map((date, i) => {
          const wd = new Intl.DateTimeFormat(i18n.language, { weekday: 'narrow' }).format(date);
          return (
