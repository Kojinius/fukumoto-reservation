...
async function loadClinicSettings() {
    ...
    if (snap.exists()) {
        ...
        if (ph) {
            ...
            if (hPhone) hPhone.innerHTML = `<a href="tel:${ph.replace(/[^\d+]/g, '')}" aria-label="電話する">☎ ${ph}</a>`;
...
async function loadJapaneseFont(pdfDoc) {
    if (!_fontCache) {
        _fontCache = await fetch('/apps/FukumotoGroup/OnlineAppointSystem/fonts/NotoSansJP-Regular.ttf').then(r => r.arrayBuffer());
    }
